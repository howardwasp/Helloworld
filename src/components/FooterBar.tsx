...
export function FooterBar({ dash }: { dash: DashboardModel }) {
  ...
  const live = dash.sources.filter(
    (source) => (source.mode === 'live' || source.mode === 'cached') && source.id !== 'news',
  )
  const sample = dash.sources.filter(
    (source) => (source.mode === 'sample' || source.mode === 'fallback') && source.id !== 'news',
  )
  ...
  return (
    <footer className="flex min-h-10 shrink-0 flex-wrap items-center gap-x-4 gap-y-1 border-t border-[#e4dfd4] bg-white px-3 py-1.5 text-[11px] text-[#6d675c]">
      ...
      <span>
        {live.length
          ? `Live · ${live
              .map((source) => `${source.provider.split(' / ')[0]}${source.mode === 'cached' ? ' (cached)' : ''}`)
              .join(' · ')}`
          : 'Connecting…'}
      ...
      <span className="ml-auto text-right">
        ...
        <span className="hidden lg:inline">
          {' '}
          · USGS · NWS · GDELT · OFAC · IODA · Open-Meteo (CC BY 4.0) · Natural Earth
        </span>
      </span>
    </footer>
  )
